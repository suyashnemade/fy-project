...
│   ├── features/          # Feature logic (Image, text, and video search)
│   ├── clip_model.py      # OpenAI CLIP wrapper
│   ├── clustering.py      # Image clustering functionality
│   ├── explainability.py  # Visual heatmaps for explainability
...
│   └── src-tauri/         # Tauri Rust configuration for standalone desktop build
...
├── evaluation/            # Notebooks and scripts for evaluating model performance
├── fine_tuning/           # Scripts for fine-tuning the CLIP model
├── storage/               # Autogenerated: FAISS index, metadata, feedback DB
├── tests/                 # Unit testing suite
├── build_backend.ps1      # Script to package the FastAPI backend as a Tauri sidecar
